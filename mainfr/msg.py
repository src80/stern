...
from otherfs.rank import setrank ,isrank ,remrank ,setsudos ,remsudos ,setsudo,IDrank,GPranks
from otherfs.send import send_msg, BYusers, sendM,Gcommands,GetLink
from mainfr.delete import delete
from otherfs.tg import Bot, Ckuser
from mainfr.ranks import ranks
from mainfr.locks import locks
from mainfr.gpcmd import gpcmd
from mainfr.sudo import sudo
from mainfr.all import allGP
from otherfs.tg import Bot,Del24
from config import *
...
def updateHandlers(client, message,redis):
		...
		if text and group is False:
			if (rank is "sudo" or rank is "sudos" or rank is "asudo") or (redis.get("{}Nbot:autoaddbot".format(BOT_ID)) and GPranks(userID,chatID) == "creator"):
				...
				if text == c.add and not redis.sismember("{}Nbot:disabledgroups".format(BOT_ID),chatID) and Ckuser(message):
					...
					kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="https://t.me/GO_XXO")]])
					...
					get = (redis.hget("{}Nbot:links".format(BOT_ID),chatID) or GetLink(chatID) or "https://https://t.me/GO_XXO")
	...
	if type is enums.ChatType.PRIVATE and message.outgoing != True:
		...
		if text and re.search("^/start$",text):
			...
			kb = InlineKeyboardMarkup([[InlineKeyboardButton("قناة السورس ", url="https://t.me/GO_XXO")]])
