...
from otherfs.send import send_msg, BYusers, Sendto, fwdto,Name,Gcommands,getAge
...
def updateCallback(client, callback_query,redis):
  ...
  c = importlib.import_module("commands.command")
  r = importlib.import_module("commands.commandreply")
  ...
  if group is True and int(date[2]) == userID and not redis.get("{}Nbot:floodUsers:{}".format(BOT_ID,userID)):
    ...
    if date[0] == "Chcommands":
      name = date[1]
      redis.srem("{}Nbot:commands:ar".format(BOT_ID),chatID)
      redis.srem("{}Nbot:commands:arem".format(BOT_ID),chatID)
      redis.srem("{}Nbot:commands:en".format(BOT_ID),chatID)
      redis.sadd("{}Nbot:commands:{}".format(BOT_ID,name),chatID)
      
      Bot("editMessageReplyMarkup",{"chat_id":chatID,"message_id":message_id,"disable_web_page_preview":True,"reply_markup":Ccommands(client, callback_query,redis,r)})
    ...
    if date[0] == "ShowDateUser":
      ...
      age = getAge(userID,r)
      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(Name(userFN),url="t.me/GO_XXO")],[InlineKeyboardButton(r.Rrank.format(t),url="t.me/GO_XXO")],[InlineKeyboardButton(r.Rmsgs.format(msgs),url="t.me/GO_XXO")],[InlineKeyboardButton(r.Rrate.format(str(rate)+"%"),url="t.me/GO_XXO")],[InlineKeyboardButton(r.Redits.format(edits),url="t.me/GO_XXO")],[InlineKeyboardButton(r.Rage.format(age),url="t.me/GO_XXO")]])
