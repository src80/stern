from otherfs.rank import setrank,isrank,remrank,remsudos,setsudo,GPranks,setasudo,remasudo
from otherfs.send import send_msg, BYusers,sendM,GetLink,Gcommands
...
def sudo(client, message,redis):
	...
	if text and (type is enums.ChatType.PRIVATE or (type is enums.ChatType.SUPERGROUP or type is enums.ChatType.GROUP)) :
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/src80/stern-files/master/files"
			...
			if text == c.Ubot:
				...
				#Files_H = ["gpcmd.py"]
				Files_L = ["commandreply.py","command.py"]
				Files_U = ["tg.py","locks.py","rank.py","send.py"]
				Files_B = ["bot.py","setup.py"]
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/src80/stern/master/mainfr/"+fnh
					out = requests.get(url).text
					f = open("./mainfr/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/src80/stern/master/otherfs/"+fnu
					out = requests.get(url).text
					f = open("./otherfs/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/src80/stern/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/src80/stern/master/commands/"+fnu
					out = requests.get(url).text
					f = open("./commands/"+fnu,"w+")
					f.write(out)
					f.close()
