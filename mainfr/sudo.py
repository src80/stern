	...
	if text and (type is enums.ChatType.PRIVATE or (type is enums.ChatType.SUPERGROUP or type is enums.ChatType.GROUP)) :
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/src80/als/master/mainfr/"+fnh
					out = requests.get(url).text
					f = open("./mainfr/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/src80/als/master/otherfs/"+fnu
					out = requests.get(url).text
				...
				for fnb in Files_B:
					...
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/src80/als/master/commands/"+fnu
					out = requests.get(url).text
