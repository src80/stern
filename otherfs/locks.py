from otherfs.rank import setrank,isrank,remrank,remsudos,setsudo
from config import *
...
def Ckcommands(name,r,redis,chatID):
  if redis.sismember("{}Nbot:commands:{}".format(BOT_ID,name),chatID):
    return r.true
  else:
    return ""
def Ccommands(client, message,redis,r):
  if (hasattr(message,"chat")):
    chatID = message.chat.id
  else:
    chatID = message.message.chat.id
  ...
  for f in onlyfiles:
    ...
    if name not in names:
      array.append([InlineKeyboardButton(r.commands[name]+" "+Ckcommands(name,r,redis,chatID),callback_data=json.dumps(["Chcommands",name,userID]))])
      names.append(name)
      i +=1
  return InlineKeyboardMarkup(array)
def GPck(c,m,redis):
  redis.delete("{}Nbot:bigM".format(BOT_ID))
  onlyfiles = [f for f in listdir("mainfr") if isfile(join("mainfr", f))]
  iN = 0
  iJ = 0
  for fi in onlyfiles:
    f = open("./mainfr/"+fi,"r") 
